...
require.config({
	shim :　{
		jquery : {
			...
			//其等效于在function()的注入的$参数。即：define(['jquery'],function($){})
	    	exports: '$'
		},
		jqueryui : {
	        exports: "$",
	        //一旦加载本模块，就会同事加载其依赖的模块
	        //其等效于当加载jqueryui时，同时加载jquery。即:define(['jqueryui','jquery'],function($,$){})
	        deps: ['jquery']
	    },
	    angular : {
			exports:'angular'
...
**更进一步！** 对于需要初始化的模块，我们可以加入init，承接加载后的初始化工作。以加载dynatree为例子：
```
require.config({
	shim :　{
		jquery : {
